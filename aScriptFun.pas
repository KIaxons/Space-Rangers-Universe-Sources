...
uses SysUtils,Classes,Math,
     ...
     SE_Ruins,SE_Planet,fStarMap,fShip2,ab_ShipAI,ab_W,EC_Mem, aGalaxyEvent, aGalaxyStruct,
...
begin
  ...
  Galaxy.AddPlanetNews(dsGalaxyNews,av[1].VStr);
end;
...
begin
  ...
  begin
    min:=av[1].VInt;
    max:=av[2].VInt;
    av[0].VInt:=min+Random(max-min);
  end else av[0].VInt:=RndOut(av[1].VInt,av[2].VInt,Galaxy.FRndOut);
end;
...
begin
  ...
  av[0].VInt:=100-Round((ship.FHull.FHitPoints/ship.FHull.FSize)*100);
end;
...
begin
  ...
  av[0].VInt:=ship.FHull.FHitPoints;
end;
...
begin
    ...
    if ship.FCustomTypeName='' then av[0].VStr:=ship.TypeName
    else av[0].VStr:=ship.FCustomTypeName;
...
begin
    ...
    if High(av)>1 then ship.FRankPoints:=word(av[1].VInt);
end;
...
begin
	...
	for i:=0 to Galaxy.FStars.Count-1 do begin
    	star:=Galaxy.FStars.Items[i];
		for u:=0 to star.FShips.Count-1 do begin
        	ship:=star.FShips.Items[u];

            if (st=1) and (ship.FShipType=t_Kling) and (TKling(ship).FSeries=t_Blazer) and (TKling(ship).FType<>KlingBoss) then ship.FShipDestroy:=true;
        end;
    end; 
end;
...
begin
    ...
    tstr:=av[1].VStr;
    cnt:=(High(av)+1-2) div 2;
    for i:=0 to cnt-1 do begin
        tstr:=StringReplaceEC(tstr,av[2+i*2+0].VStr,ColorStr(av[2+i*2+1].VStr,txtStandart));
    end;
...
begin
    ...
    if value>0 then costChange:=value*mGoods[it].AverageCost
...
begin
    ...
    av[0].VInt:=SctiptGetSS(TShip(av[1].VDW),GScriptCur).FGroup;
end;
...
begin
    ...
    sstate:=SctiptGetSS(TShip(av[1].VDW),GScriptCur).FState;
...
begin
  ...
  if High(av)>1 then kling.FRunProgrammName := TProgramms(av[2].VDW);
end;
...
begin
    ...
    if Blazer<>nil then Blazer.CalcStanding;
end;
...
var
  ...
  item: TItem;
begin
  if(High(av) < 2) then raise Exception.Create('Error.Script SF_LinkItemToScript');
...
begin
  ...
  obj:=TObject(av[1].VDW);
  datatype:=0;
  if(High(av) > 1) then datatype:=av[2].VInt;
  av[0].VInt:=0;
...
begin
  ...
  av[0].VDW:=0;
  if(ship <> nil) then begin
    if(no >= 0) and (no < ship.FEquipments.Count) then
    begin
      item:=TItem(ship.FEquipments.Items[no]);
      av[0].VDW:=Cardinal(item);
      if(item is TEquipment) and (item as TEquipment).FExplotable then
      begin
...
begin
  ...
  if(ship <> nil) then
  begin
    if(no >= 0) and (no < ship.FArtefacts.Count) then
    begin
...
begin
  ...
  if(obj is TPlanet) then begin
    ...
    if(no >= 0) and (no < (obj as TPlanet).FEquipmentShop.Count) then begin
      av[0].VDW:=Cardinal((obj as TPlanet).FEquipmentShop.Items[no]);
      (obj as TPlanet).FEquipmentShop.Delete(no);
    end;
    if updateshop then begin
      EquipmentShopToShopList(); // ????????????? ???? ? ???????
      GFormEquipmentShop.FreeImage();
    ...
    if updateshop then fEquipmentShop.EquipmentShopToShopList(); // ????????????? ???? ? ????
    if(no >= 0) and (no < (obj as TRuins).FEquipmentShop.Count) then begin
      av[0].VDW:=Cardinal((obj as TRuins).FEquipmentShop.Items[no]);
      (obj as TRuins).FEquipmentShop.Delete(no);
    end;
    if updateshop then begin
      EquipmentShopToShopList(); // ????????????? ???? ? ???????
      GFormEquipmentShop.FreeImage();
...
begin
  ...
  if(obj <> nil) then begin
    if(High(av) = 1) then begin
      if(GShopList <> nil) and ((Player.FCurPlanet = obj) or (Player.FCurShip = obj)) then begin
        for i:=0 to GShopList.Count-1 do begin
        ...
        if(obj is TPlanet) then begin
          for i:=0 to (obj as TPlanet).FEquipmentShop.Count-1 do begin
            if((obj as TPlanet).FEquipmentShop.Items[i] = nil) then continue;
             inc(cnt);
          end;
        end else if(obj is TRuins) then begin
          for i:=0 to (obj as TRuins).FEquipmentShop.Count-1 do begin
            if((obj as TRuins).FEquipmentShop.Items[i] = nil) then continue;
             inc(cnt);
          end;
        end;
      end;
      ...
      no:=av[2].VInt;
      if(GShopList <> nil) and ((Player.FCurPlanet = obj) or (Player.FCurShip = obj)) then begin
        if(no >= 0) and (no < GShopList.Count) then av[0].VDW:=Cardinal(TShopSlot(GShopList.Items[no]).FEq) else av[0].VDW:=0;
      end else begin
        if(obj is TPlanet) then begin
          if(no >= 0) and (no < (obj as TPlanet).FEquipmentShop.Count) then av[0].VDW:=Cardinal((obj as TPlanet).FEquipmentShop.Items[no]) else av[0].VDW:=0;
        end else if(obj is TRuins) then begin
          if(no >= 0) and (no < (obj as TRuins).FEquipmentShop.Count) then av[0].VDW:=Cardinal((obj as TRuins).FEquipmentShop.Items[no]) else av[0].VDW:=0;
        end;
      end;
    end;
  end else av[0].VDW:=0;
...
begin
  ...
  New(dlgovd);
  dlgovd.FDialog:=av[1].VStr;
  dlgovd.FPriority:=av[2].VInt;
  dlgovd.FScript:=Cardinal(GScriptCur);
...
begin
  ...
  shipab.FIsPartner:=(High(av) > 1) and (av[2].VInt = 1);
  ...
  begin
    ...
    if obj is TScriptItem then shipab.FCustomDrop:=TScriptItem(obj).FItem
...
begin
  ...
  begin
    ...
    begin
      ...
      abWaeponInitStd(@(shipab.FWeapon[i]), w.FItemType);
...
begin
  ...
  if(ship <> nil) then begin
    ...
    if(High(av) > 1) then ship.FFace:=av[2].VInt;
  end;
end;
...
var
  ...
  ship: TShip;
begin
  if(High(av) < 3) then raise Exception.Create('Error.Script OrderMove');
  absolut:=false;
...
begin
  ...
  if(arrnum > 1) then begin
    for i:=2 to arrnum do begin
      if(av[1].VType <> vtArray) then raise Exception.Create('Error.Script ArraySort - type mismatch');
...
begin
  ...
  if(star.FStatus.Owners = Klings) or (star.FStatus.CustomFaction <> '') then begin av[0].VInt:=3; exit; end;
  if star.FStatus.Battle then begin av[0].VInt:=2; exit; end;
  av[0].VInt:=0;

  if Galaxy.FKellerAttackStar=star then av[0].VInt:=1;

  for j:=0 to star.FShips.Count-1 do
    if TShip(star.FShips.Items[j]).FCurStanding in [tst_Kling,tst_Custom] then av[0].VInt:=1;

  i:=1;
  while ((i<Galaxy.FStars.Count) and (av[0].VInt=0)) do begin
    star2:=TStar(star.FStarDist[i].FStar);
    if(star2.FStatus.Owners = Klings) or (star.FStatus.CustomFaction <> '') then
    begin
      for j:=0 to star2.FShips.Count-1 do
        if(TShip(star2.FShips.Items[j]).FCurStanding in [tst_Kling,tst_Custom]) and (TShip(star2.FShips.Items[j]).FOrderObj = star) then
        begin
          av[0].VInt:=1;
          exit;
        end;
...
begin
  ...
  ship.FScriptOrderAbsolute:=scriptorder;
  pos:=ship.CalcAfterJumpPoint(starto);
  ship.FOrderDes:=pos;
  ship.FPos.X:=pos.X;
  ship.FPos.Y:=pos.Y;
...
begin
  ...
  end;//0 - normal, 1 - no attack (except between hostile factions), 2 - no attack from AI including assists, 3 - no robbing from pirates and rangers
end;
...
begin
  ...
  obj:=TObject(av[1].VDW);
  if(obj is TPlanet) then begin
    if (obj as TPlanet).FFlag_NoShopUpdate then av[0].VInt:=1;
    if(High(av) > 1) then (obj as TPlanet).FFlag_NoShopUpdate:=(av[2].VInt <> 0);

  end else if(obj is TRuins) then begin
    if (obj as TRuins).FFlag_NoShopUpdate then av[0].VInt:=1;
    if(High(av) > 1) then (obj as TRuins).FFlag_NoShopUpdate:=(av[2].VInt <> 0);
  end;
end;
...
begin
  ...
  av[0].VStr:='';
  no:=av[1].VInt;
  if(no >= 0) and (no < Galaxy.FPlanetNews.Count) then av[0].VStr:=PPlanetNews(Galaxy.FPlanetNews.Items[no]).FText;
end;
...
begin
  ...
  if (High(av) >= 10) then color:=GR_PF.Color(av[8].VInt, av[9].VInt, av[10].VInt)
    ...
    begin
      ...
      else if TShip(target).IsCustomFaction then color := FactionColor(TScriptShip(TShip(target).FScriptShip).FCustomFaction)
...
begin
  ...
  if GScriptCur<>nil then locVar:=GScriptCur.FCodeInit.LocalVar else locVar:=nil;

  if High(av) = 1 then
  begin
    InterpretAndExecuteString(tstr,code);
    exit;
  end;
  ...
  if locVar<>nil then tempCode.LinkAll(locVar);

  cnt:=(High(av)-1) div 2;

  for i:=1 to cnt do
    tempCode.LocalVar.Add( av[2*i].VStr, av[2*i+1].VType).Assume2( av[2*i+1] );
  ...
  begin
    av[0].Assume2( tempCode.LocalVar.GetVar( av[High(av)].VStr ) );
  end;
...
begin
  ...
  for i:=0 to cnt-1 do
  begin
    if bpp.Block_Count(GetStrParEC(path,i,'.')) = 0 then exit;
...
begin
  ...
  begin
    ...
    if TMessageLoopGI(GForm[form]).FName<>formName then continue;
    obj:=TMessageLoopGI(GForm[form]).GetByNameNE(objName);
    break;
  end;
...
begin
  ...
  begin
    ...
    if High(av) > 2 then
    begin
      if obj is TgaiGI then TgaiGI(obj).Image:=av[3].VStr
...
begin
  ...
  if High(av) > 1 then
  begin
    if obj is TgaiGI then TgaiGI(obj).Image:=av[2].VStr
...
begin
  ...
  va.Add('ForPlanetBattle',vtInt).VInt:=integer(ForPerfidy);
